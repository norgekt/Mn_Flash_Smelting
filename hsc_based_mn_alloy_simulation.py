"""
HSC-Based Conceptual Mn Alloy Process Simulation
Two-stage hydrogen-based flash smelting process for ferromanganese alloy production
This script is based on the conceptual_mn_alloy_process_simulation.py, but updated
to use parameters and produce plots based on the HSC_SIM_Model.csv and HSC_SIM.jpg.
"""
...
class ProcessParameters:
    ...
    # Stage 1 Parameters (H2 Reduction) - Based on HSC_SIM model
    temperature_stage1: float = 1273.0  # K (1000°C)
...
class ManganesReductionSimulator:
    ...
    def run_stage2_simulation(self, stage1_output: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        if self.dnn_model is None:
            ...
            results = self.stage2_dnn_prediction(stage1_output)

        self.stage2_results = results
        return results

    def stage2_dnn_prediction(self, stage1_output: Dict[str, float]) -> Dict[str, float]:
    ...
    def plot_flowsheet(self, save_plots: bool = True):
        """Generate a flowsheet-style plot of the process."""
        if self.stage1_results is None or self.stage2_results is None:
            print("No results to plot. Run both simulations first.")
            return
        ...
        # Box properties
        box_props = dict(boxstyle='round,pad=0.5', fc='lightblue', ec='b', lw=1)
        process_box_props = dict(boxstyle='round,pad=0.5', fc='lightcoral', ec='r', lw=1)

        # Helper function to create a formatted string for the boxes
        def format_box_text(title, data):
            # Matplotlib doesn't support markdown bold, so we'll just make the title a separate text object
            text = ""
        ...
        # --- Define data for each box from simulation results ---
        s1_out = self.stage1_results.iloc[-1]
        s2_out = self.stage2_results

        mn_ore_data = {"Mass Flow (kg/h)": 1000.0, "Temperature (°C)": 25, "Mn2O3 (wt-%)": 75.0}
        h2_data = {"Volumetric Flow (Nm³/h)": 200, "Temperature (°C)": 25}

        # Stage 1 outputs
        reduced_mn_ore_mass = s1_out['MnO'] + s1_out['Mn3O4'] + s1_out['MnO2'] + s1_out['Mn2O3']
        off_gas_h2o = s1_out['H2O']
        off_gas_h2 = s1_out['H2']

        reduced_mn_ore_data = {"Mass Flow (kg/h)": reduced_mn_ore_mass, "Temperature (°C)": 1000}
        off_gas_data = {"Volumetric Flow (Nm³/h)": 200, "Temperature (°C)": 1000, "H2O (kg)": off_gas_h2o, "H2 (kg)": off_gas_h2}

        # Stage 2 inputs and outputs
        al_alloy_data = {"Mass Flow (kg/h)": 194.4, "Temperature (°C)": 200}
        flux_data = {"Mass Flow (kg/h)": 50.0, "Temperature (°C)": 25}
        mn_alloy_data = {"Mass Flow (kg/h)": s2_out.get('Mn_produced', 0), "Temperature (°C)": 1550}
        slag_data = {"Mass Flow (kg/h)": s2_out.get('Al2O3_produced', 0), "Temperature (°C)": 1550}

        # --- Draw boxes and text ---
        positions = {
            "Mn Ore": (2, 10), "H2": (2, 7), "Pre-reduction": (6, 8.5), "Off-gas": (10, 10),
            "Reduced Mn Ore": (10, 7), "Smelting Reduction": (10, 4.5), "Al Alloy": (6, 2),
            "Flux": (2, 4.5), "Mn Alloy": (14, 6), "Slag": (14, 3)
        }

        box_data = {
            "Mn Ore": mn_ore_data, "H2": h2_data, "Off-gas": off_gas_data,
            "Reduced Mn Ore": reduced_mn_ore_data, "Al Alloy": al_alloy_data,
            "Flux": flux_data, "Mn Alloy": mn_alloy_data, "Slag": slag_data
        }

        for name, pos in positions.items():
            is_process = "reduction" in name.lower()
            props = process_box_props if is_process else box_props
            ax.text(pos[0], pos[1], name, ha="center", va="center", bbox=props, fontsize=10, weight='bold')
            if name in box_data:
                 ax.text(pos[0], pos[1]-0.8, format_box_text("", box_data[name]), ha="center", va="center", fontsize=8)

        # --- Draw arrows ---
        def draw_arrow(start, end):
            ax.annotate("", xy=positions[end], xytext=positions[start],
                        arrowprops=dict(arrowstyle="->", connectionstyle="arc3", lw=1.5, color="gray"))

        draw_arrow("Mn Ore", "Pre-reduction")
        draw_arrow("H2", "Pre-reduction")
        draw_arrow("Pre-reduction", "Off-gas")
        draw_arrow("Pre-reduction", "Reduced Mn Ore")
        draw_arrow("Reduced Mn Ore", "Smelting Reduction")
        draw_arrow("Al Alloy", "Smelting Reduction")
        draw_arrow("Flux", "Smelting Reduction")
        draw_arrow("Smelting Reduction", "Mn Alloy")
        draw_arrow("Smelting Reduction", "Slag")

        plt.tight_layout()
        if save_plots:
            if not os.path.exists('simulation_plots'):
                os.makedirs('simulation_plots')
            plt.savefig('simulation_plots/flowsheet.png', dpi=300)
            print("Flowsheet plot saved to simulation_plots/flowsheet.png")
        plt.show()
...
def main():
    ...
    print("=== HSC-BASED MN ALLOY PROCESS SIMULATION ===\n")
    ...
    # Define initial ore composition from HSC_SIM_Model.csv
    initial_ore = {
        'Mn2O3': 750.0,  # kg (1000kg ore * 75% Mn2O3)
        'MnO2': 0.0,
        'Mn3O4': 0.0,
        'MnO': 0.0,
    }
    ...
    # Create plots
    simulator.plot_flowsheet(save_plots=True)
